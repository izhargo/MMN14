...
char *isInstruction(char *str)
{
	...
	{
		if (strcmpci(str,instructions[i]) == 0)
		{ 
			/*str is an instructions name*/
...
/*A function to check if a string is a valid label if not add error and return NULL*/
...
{	
	...
	{ 
		/*direct address, cannot validate in first transition*/
		result++;
	}
	return result;
}
...
{	
	...
	if (str[0]=='#')
	{
		/*immediate address (0) not allowed*/
		addError(WRONG_OPERAND_INPUT,lineNum);			
		return 0;	
	}
	else if (isRegister(str))
...
int calcOneOperand12(char *str, int lineNum)
{
	...
	{ 
		/*register address (3), not allowed*/
		addError(WRONG_OPERAND_INPUT,lineNum);			
		return 0;
	}
	else if (strchr(str,'[') != NULL) 
...
void noOperand (char *line, int lineNum)
{
	...
	{ 
		/*we have a kind of an operand - and we shouldn't*/
		addError(TOO_MANY_OPERANDS,lineNum);			
		return;
	}
	return;
...
/*A function to analize a line in the first transition, check any error and add symbols to the symbols list*/
void analizeLine(char *line, int lineNum)
{
	...
	{ 
		...
		{		
			if (flagLabel) 
				addToSymbolList(&SymbolTable, &SymbolTableLast, label, DC, 0, 0, 0);				
	...
	else if (isOpCode(token) != NULL)
	{
		...
		if (flagLabel) 
			addToSymbolList(&SymbolTable, &SymbolTableLast, label, IC, 1, 0, 0);	
